...
knnresult distrAllkNN(double* Y, int n, int d, int k) // reversed symbols (Y == X) for clarity of code
{
	...
	MPI_Request req1,req2; //request for the handler
	...
	for (int i = 1; i < numprocs; i++)
	{
		...
		{
			if (rank != numprocs - 1)
				MPI_Isend(X, block * d, MPI_DOUBLE,
				...
				MPI_Isend(X, block * d, MPI_DOUBLE,
					...
					&req1);
			if (rank != 0)
				MPI_Irecv(temp, block * d, MPI_DOUBLE,
					rank - 1, 1, MPI_COMM_WORLD,
					&req2);
			else
				MPI_Irecv(temp, block * d, MPI_DOUBLE,
					numprocs - 1, 1, MPI_COMM_WORLD,
					&req2);

			tempknn = kNN(X, Y, n, n, d, k);
		...
		{
			if (rank != numprocs - 1)
				MPI_Isend(Y, block * d, MPI_DOUBLE,
					rank + 1, 1, MPI_COMM_WORLD,
					&req1);
			else
				MPI_Isend(Y, block * d, MPI_DOUBLE,
					...
					&req1);
			if (rank != 0)
				MPI_Irecv(X, block * d, MPI_DOUBLE,
					rank - 1, 1, MPI_COMM_WORLD,
					&req2);
			else
				MPI_Irecv(X, block * d, MPI_DOUBLE,
			...
			knnres = kNN(Y, Y, block, block, d, k); // knn init

			//offsetting the indices of the initial knn result
			if (rank > 1)
			{
	...
	//setting the local maximum-minimum
	for (int i = 0; i < n * k; i++)
	{
		if (knnres.ndist[i] > maxDist)
			maxDist = knnres.ndist[i];
		if (knnres.ndist[i] < minDist && knnres.ndist[i]>1e-5)
	...
	free(X);
	free(temp);
	//finding the global minimum distance and sending it to master process
